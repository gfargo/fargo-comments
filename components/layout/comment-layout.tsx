...
interface CommentVariantContextType {
  selectedVariant: CommentVariant
  setSelectedVariant: (variant: CommentVariant) => void
}

const CommentVariantContext = createContext<CommentVariantContextType | undefined>(undefined)

export function useCommentVariant() {
  const context = useContext(CommentVariantContext)
  if (!context) {
    throw new Error("useCommentVariant must be used within a CommentLayout")
  }
  return context
}
...
export function CommentLayout({ children, title, description }: CommentLayoutProps) {
  const [selectedVariant, setSelectedVariant] = useState<CommentVariant>("card")
  const { toast } = useToast()
  const { clearAllStorage } = useComments()
  ...
  return (
    <CommentVariantContext.Provider value={{ selectedVariant, setSelectedVariant }}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 px-2 py-1 bg-gray-50 rounded-md border">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSelectedVariant(value as CommentVariant)}
                  >
    ...
    </CommentVariantContext.Provider>
  )
}
