...
import { $getSelection, $isRangeSelection, type TextNode, COMMAND_PRIORITY_LOW, KEY_DOWN_COMMAND } from "lexical"
    ...
    () => (event: KeyboardEvent) => {
      if (showResults) {
        ...
        if (event.key === "ArrowDown") {
          ...
          setSelectedIndex((prev) => Math.min(prev + 1, emojiResults.length - 1))
          return true
        }
      ...
      if (showResults && event.key.length === 1 && !event.ctrlKey && !event.metaKey && event.key !== ":") {
        setTimeout(() => {
          try {
            editor.getEditorState().read(() => {
              if (!triggerNode) return
    ...
    >
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden min-w-[200px] max-w-[300px]">
        <div className="p-2 text-xs text-gray-500 border-b">
          {searchQuery ? `Search: "${searchQuery}"` : "Type to search emojis..."}
        </div>
          ...
          {emojiResults.map((emoji, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-900 truncate">{emoji.name}</div>
                {emoji.keywords.length > 0 && (
                  <div className="text-xs text-gray-500 truncate">{emoji.keywords.slice(0, 3).join(", ")}</div>
                )}
              </div>
            </div>
          ))}
        </div>
        <div className="p-2 text-xs text-gray-400 border-t">↑↓ navigate • Enter select • Esc cancel</div>
      </div>
    </div>
  )
}
